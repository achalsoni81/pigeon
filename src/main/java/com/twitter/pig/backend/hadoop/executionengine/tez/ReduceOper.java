...
import com.twitter.pig.backend.hadoop.executionengine.mapReduceLayer.PigIntermediateReduce;
...
public class ReduceOper extends TezOperator {
	...
	private boolean needsDistinctCombiner = false;
	...
	private String customPartitioner = null;
	...
	private boolean isIntermediateReducer = false;

	private boolean isEndOfAllInputSetInReduce;
	...
	public TezConfiguration configure(PigContext pigContext, Configuration conf) {
		...
		try{
			...
			//Process the POLoads
			List<POLoad> lds = PlanHelper.getPhysicalOperators(plan, POLoad.class);
			...
			if(lds!=null && lds.size()>0){
				for (POLoad ld : lds) {
					...
					//from this input
					List<PhysicalOperator> ldSucs = plan.getSuccessors(ld);
			...
			if (isIntermediateReducer) {
				nwJob.setReducerClass(PigIntermediateReduce.class);
			...
			if(!pigContext.inIllustrator) {
				...
				conf.set("pig.reducePlan", ObjectSerializer.serialize(getPlan()));
			}
			...
			if (!pigContext.inIllustrator)
			{
				...
				conf.set(TezJobControlCompiler.PIG_MAP_STORES, ObjectSerializer.serialize(new LinkedList<POStore>()));
                conf.set(TezJobControlCompiler.PIG_REDUCE_STORES, ObjectSerializer.serialize(stores));
			}
	...
	public void configureVertex(Vertex vertex, Configuration conf, Map<String, LocalResource> commonLocalResources, Path remoteStagingDir){
		vertex.setJavaOpts(MRHelpers.getReduceJavaOpts(conf));
		vertex.setTaskLocalResources(commonLocalResources);
		Map<String, String> reduceEnv = new HashMap<String, String>();
		MRHelpers.updateEnvironmentForMRTasks(conf, reduceEnv, false);
	...
	public boolean hasOrderBy() {
		...
		if (isGlobalSort || isLimitAfterSort|| usingTypedComparator) {
			hasOrderBy = true;
		} else {
			/*
	...
	public boolean isEndOfAllInputSetInReduce() {
		return isEndOfAllInputSetInReduce;
	}

	public void setEndOfAllInputSetInReduce(boolean isEndOfAllInputSetInReduce) {
		this.isEndOfAllInputSetInReduce = isEndOfAllInputSetInReduce;
	}

	public void setUDFs(Set<String> uDFs2) {
		this.UDFs = UDFs;
	}
	...
	public boolean isNeedsDistinctCombiner() {
		return needsDistinctCombiner;
	}

	public void setNeedsDistinctCombiner(boolean needsDistinctCombiner) {
		this.needsDistinctCombiner = needsDistinctCombiner;
	}
	...
	public void setIsIntermediateReducer(boolean isIntermediateReducer) {
		this.isIntermediateReducer = isIntermediateReducer;
	}
	
	public boolean isIntermediateReducer() {
		return isIntermediateReducer;
	}
